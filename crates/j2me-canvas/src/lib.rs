...
// WoS-contributed tests (ported verbatim from the pre-migration `hlws-canvas`,
// adapted only to the canonical fallible `create_mutable`/`from_argb` API). The
// canonical rounded `source_over` satisfies the same inequalities WoS asserted.
#[cfg(test)]
mod contributed_tests {
    use super::*;

    #[test]
    fn mutable_image_starts_opaque_white() {
        let img = Image::create_mutable(4, 3).unwrap();
        assert_eq!(img.width(), 4);
        assert_eq!(img.height(), 3);
        assert!(img.is_mutable());
        assert_eq!(img.get(0, 0), Some(0xFFFF_FFFF));
        assert_eq!(img.get(4, 0), None); // out of bounds
    }

    #[test]
    fn set_clips_silently_out_of_bounds() {
        let mut img = Image::create_mutable(2, 2).unwrap();
        img.set(-1, 0, 0xFF00_0000);
        img.set(0, 5, 0xFF00_0000);
        img.set(1, 1, 0xFF00_00FF);
        assert_eq!(img.get(1, 1), Some(0xFF00_00FF));
    }

    #[test]
    fn source_over_fast_paths_and_blend() {
        assert_eq!(source_over(0xFF12_3456, 0x0000_0000), 0xFF12_3456); // opaque src
        assert_eq!(source_over(0x0012_3456, 0xFFAB_CDEF), 0xFFAB_CDEF); // transparent src
                                                                        // 50% red over blue -> roughly half-and-half.
        let out = source_over(0x80FF_0000, 0xFF00_00FF);
        assert_eq!((out >> 24) & 0xFF, 0xFF); // over an opaque dst stays opaque
        assert!((out >> 16) & 0xFF >= 0x7F); // meaningful red contribution
        assert!(out & 0xFF >= 0x7F); // remaining blue
    }

    #[test]
    fn blend_composites_onto_the_buffer() {
        let mut img = Image::create_mutable(2, 2).unwrap();
        // Start from an opaque blue background.
        img.set(0, 0, 0xFF00_00FF);
        // Half-alpha red over it: red rises, blue falls, stays opaque.
        img.blend(0, 0, 0x80FF_0000);
        let out = img.get(0, 0).unwrap();
        assert_eq!((out >> 24) & 0xFF, 0xFF);
        assert!((out >> 16) & 0xFF >= 0x7F);
        // Out-of-bounds blend is a silent no-op.
        img.blend(9, 9, 0xFF00_FF00);
    }
}
