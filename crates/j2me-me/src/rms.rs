...
//!
//! Only the surface WoS's `au`/`RmsFile` wrapper touches is modeled:
//! `openRecordStore(name, create)`, `closeRecordStore()`,
//! `deleteRecordStore(name)`, `getNumRecords()`, `getNextRecordID()`,
//! `getRecordSize(id)`, `getRecord(id)`, `addRecord(data, off, len)`. The game
//! never calls `setRecord`/`deleteRecord`, so they are not modeled (R: no APIs
//! the game never calls). Record IDs are **monotonic** as in MIDP — assigned by
//! `addRecord`, never reused, reset only when the store itself is deleted and
//! recreated (exactly what `RmsFile.close` does to keep one packed record).
...
impl RmsRuntime {
    ...
    /// Whether a record store with this name exists (backs `RmsFile.exists`).
...
mod tests {
    ...
    fn open_requires_existing_store_when_not_creating() {
        ...
        // Read-mode / exists-probe on an absent store throws (RmsFile.exists).
    ...
    fn add_then_read_back_round_trips_the_bytes() {
        // The RmsFile.close/read pattern: one packed record, read via
    ...
    fn record_ids_are_monotonic_and_survive_a_delete_but_reset_on_recreate() {
        ...
        // deleteRecordStore + openRecordStore(create) resets ids (RmsFile.close).
