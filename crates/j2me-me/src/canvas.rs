//! MIDP Canvas/Display state and the serialized paint/input queue.
//!
//! Canonical `j2me-me` surface adopted from the j2me home `_template/`, extended
//! with the [`Displayable`] surface trait (a WoS capability the template's runtime
//! did not carry; add-only, an upstream candidate).
...
/// `Displayable` surface. (WoS-contributed; add-only, an upstream candidate.)
...
// WoS-contributed tests (ported from the pre-migration `hlws-me::canvas`, adapted
// only to the canonical method names: `common_game_action`, `Display::default`,
// the previous/next `set_current`, `has_current`, and the `clear_current`
// hide path). Every behavioral assertion is preserved.
#[cfg(test)]
