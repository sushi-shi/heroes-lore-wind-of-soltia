...
//! malformed data.
//!
//! WoS calls exactly two decode factories:
//! - `Image.createImage(byte[] data, int off, int len)` — the sprite / merged-PNG
//!   / bitmap-font loaders ([`create_image_region`]);
//! - `Image.createImage(String name)` — a localized font sheet loaded by resource
//!   name ([`create_image_named`], resolved through the host [`ImageResources`]
//!   seam; a missing resource is an `IOException`, matching the real API).
//!
//! Decoding folds the corpus's paletted 1/2/4/8-bit PNGs (some with a `tRNS`
//! chunk) into straight 8-bit channels, packed `0xAARRGGBB` (`EXPAND | STRIP_16`).
...
/// `Image.createImage(String name)` — load a named resource (a localized font
/// sheet in WoS) and decode it. The resource is resolved through the host
...
mod tests {
    ...
    fn create_image_decodes_paletted_with_trns_via_expand() {
        // The real corpus shape: an indexed PNG with a tRNS chunk. EXPAND must
        // turn it into RGBA before packing.
        let mut out = Vec::new();
        {
