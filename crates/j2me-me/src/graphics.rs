...
//!
//! Canonical `j2me-me` surface adopted from the j2me home `_template/`
//! (`setColor`/clip/`translate`/`fillRect`/`drawRect`/`drawLine`/`drawImage`/
//! `drawRegion`, with `GraphicsError` and `SpriteTransform`), extended with the
//! MIDP `drawArc` / `fillArc` ellipse-sector rasteriser (a WoS capability the
//! template's runtime did not carry; add-only, an upstream candidate) and a public
...
impl<'a> Graphics<'a> {
    ...
    /// well-defined angular sector. (WoS-contributed; upstream candidate.)
    ...
    /// ellipse sector. (WoS-contributed; upstream candidate.)
...
// WoS-contributed tests (ported from the pre-migration `hlws-me::graphics`,
// adapted to the canonical fallible `Image`/`draw_image` API and the `color()`
// accessor). The arc tests exercise the added `draw_arc`/`fill_arc` rasteriser.
#[cfg(test)]
mod contributed_tests {
    ...
    fn fill_arc_full_sweep_fills_the_ellipse_interior_but_not_the_corners() {
        // The enemy-shadow shape: fillArc(x, y, 22, 9, 0, 360) is a filled
        // ellipse. Its area is well under the bounding box, and the box corners
        // stay unpainted (R3: proves it is an ellipse, not a rectangle).
