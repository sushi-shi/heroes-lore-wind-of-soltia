//! `javax.microedition.media` (MMAPI) — the `Player`/`Manager` state model the
//! WoS `SoundPlayer` (`ci`) / `AudioManager` drive, as idiomatic Rust whose
//! *observable* behavior matches the Java ME contract.
//!
...
//! player's lifecycle is independent; `AudioManager` decides when to stop one
//! track before starting another (same as gothic/stalker).
//!
...
//!
//! WoS's exact `Player` surface: `realize`, `prefetch`, `start`, `stop`,
//! `close`, `getState`, `setLoopCount`, `addPlayerListener`, and
//! `getControl("VolumeControl").setLevel(level)`. `deallocate` /
//! `setMediaTime` / `removePlayerListener` / `getDuration` are never called by
//! the game and are not modeled (no APIs the game never calls). All sound assets
//! are `resource:` streams, so `Manager.createPlayer(String locator)` (the `http:`
//! branch, never taken by a shipped asset) is likewise not modeled — every
//! player is created from a resource with a content type.
...
struct PlayerCell {
    ...
    volume_level: i32,
    /// Whether a `PlayerListener` is attached (`ci` registers itself but ignores
    /// the events; recorded for fidelity).
    has_listener: bool,
}
...
mod tests {
    ...
    fn lifecycle_walks_the_states_and_emits_ops() {
        // The SoundPlayer.start() path: realize -> prefetch -> start, then stop.
    ...
    fn volume_control_is_present_and_sets_a_clamped_level() {
        // ci.setVolume: getControl("VolumeControl") then setLevel(level).
        let mut m = MediaRuntime::new();
