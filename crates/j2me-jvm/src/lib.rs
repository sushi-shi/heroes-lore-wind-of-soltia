...
//!
//! ## Canonical surface vs. WoS-contributed helpers
//!
//! The upper block is the canonical `j2me-jvm` surface adopted from the j2me
//! home `_template/` (the `i32_*` / `i64_*` const-fn arithmetic, `narrow_*`,
//! `array_ref`/`array_mut`, the `Clock` family, `JavaError`). The lower block,
//! marked "WoS-contributed", carries the game-agnostic helpers the Heroes Lore:
//! Wind of Soltia strict transliteration was written against — the `ishl`/`java_div`
//! spelling of the same JVM semantics, the `jget!`/`jset!` checked-access macros,
//! and slice-typed `java_array_*` accessors. They are add-only and are candidates
//! to upstream into `_template/crates/j2me-jvm`.
...
// ===========================================================================
// WoS-contributed game-agnostic helpers (add-only; upstream candidates).
//
// These are the JVM-primitive helpers the Heroes Lore: Wind of Soltia strict
// transliteration is written against. They express exactly the same JVM
// semantics as the canonical `i32_*`/`i64_*`/`array_*` surface above, under the
// bytecode-mnemonic spelling (`ishl`, `iushr`, `java_div`, …) the transliteration
// uses, plus the `jget!`/`jset!` checked-access macros and slice-typed
// `java_array_*` accessors. They belong in the shared crate so every game can use
// them; the canonical `_template/crates/j2me-jvm` should adopt them verbatim.
// ===========================================================================
...
// The WoS-contributed helpers carry their original unit tests (ported verbatim
// from the pre-migration `hlws-jvm`), so no coverage is lost in the adoption.
#[cfg(test)]
mod contributed_tests {
    use super::*;
