...
| `EntityList.java:112` / `:140` | `reorderByDepth` unlinks `node` from its old slot **without decrementing `count`**, then, when the node slides all the way to an extreme end, calls `addFront`/`addBack` — each doing `count++`. Byte-verified against `javap -c -p aq.class` (no `isub`/`iinc` on `count` in `aq.c`, only the eight depth `iadd`s): every reorder to an end therefore **inflates `count` by one**. A reorder that lands via the mid-list splice leaves `count` correct. Latent in practice (`count` bounds iteration, where the padding is tolerated). Reproduced verbatim; paired with the preserved **asymmetry** that the toward-tail branch sets `node.removed = true` while the toward-head branch does not. |
